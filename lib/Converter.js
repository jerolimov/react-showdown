...
module.exports = function Converter(options) {
	...
	this._converter.listen('blockGamut.before', function(name, text, converter, options, globals) {
		...
		var repFunc = function (wholeMatch, match, left, right) {
			...
			var txt = wholeMatch;
			txt = left + converter.makeHtml(match) + right;
			return '\n\n¨K' + (globals.gHtmlBlocks.push(txt) - 1) + 'K\n\n';
		};

		// TODO - this will blow up in node unless we have showdown set globally
		for (var component in blockComponents) {
			// handle case with opening / closing tags
			/* eslint-disable no-useless-escape */
			text = showdown.helper.replaceRecursiveRegExp(text, repFunc, '^\s*?<' + component + '\\b[^>]*>', '</' + component + '>', 'gm');
			// handle self closing tag
			text = text.replace(new RegExp('^\s*?(<' + component + '\\b[^<>]*?\/>)', 'gm'),
	...
	var parserOptions = {
		...
		lowerCaseTags: false,
		// otherwise self-closing react components aren't recognized correctly
